...
from cote_common.common_modules_rt import denoise, handle_lerche_chroma
...
def filterchain(
    *,
    ...
    rs = Rescale(
        depth(src, 32),  # Work in 32-bit for better precision
        kernel=Bilinear(),
    ...
    # Advanced debanding
    pladeband = placebo_deband(dns, thr=1.5, iterations=16)
    deband = pladeband.std.MaskedMerge(
        chromaign, dre_edgemask.RETINEX(chromaign, brz=10 / 255)
    )
